/**
 * AJUSTE DE ACTIVIDADES MANTENIMIENTO TIPO B
 * ===========================================
 * Aplica los mismos ajustes lógicos que Tipo A:
 * 1. Actividades de nivel/porcentaje → MEDICION
 * 2. Actividades incorrectas con porcentaje → INSPECCION (B/M/C/NA)
 * 3. Actividades SÍ/NO → VERIFICACION
 * 
 * Ejecutar: npx ts-node packages/database/ajustar-actividades-tipo-b.ts
 */
...
async function main() {
    try {
        separator('ANÁLISIS Y AJUSTE DE ACTIVIDADES TIPO B');

        // 1. Obtener ID del tipo de servicio GEN_PREV_B
        const tipoServicio = await prisma.tipos_servicio.findFirst({
            where: { codigo_tipo: 'GEN_PREV_B' }
        });

        if (!tipoServicio) {
            log('No se encontró el tipo de servicio GEN_PREV_B', 'error');
            return;
        }

        log(`Tipo de servicio encontrado: ${tipoServicio.nombre_tipo} (ID: ${tipoServicio.id_tipo_servicio})`, 'success');

        // 2. Obtener todas las actividades del Tipo B
        ...
        separator('PASO 1: ACTIVIDADES DE NIVEL/PORCENTAJE - CAMBIAR A MEDICION');
        ...
        separator('PASO 3: ACTIVIDADES SÍ/NO → VERIFICACION');
