 ...
 * ============================================================================
 * Script de prueba funcional con datos reales
 * 
 * Ejecutar con: node src/testing/test-fase3-simple.js
 * ============================================================================
 */
...
async function testEmailWithPDF(pdfBuffer) {
...
async function testOrdenServicio() {
  console.log('\n📝 TEST 5: Verificando datos de órdenes de servicio...');
  
  try {
    // Obtener una orden existente con relaciones correctas
    ...
    if (orden) {
      ...
      console.log(`   📊 Estado: ${orden.estado?.nombre_estado || 'N/A'}`);
      console.log(`   👤 Cliente: ${orden.cliente?.persona?.nombres || 'N/A'}`);
      console.log(`   🔧 Equipo: ${orden.equipo?.nombre_equipo || 'N/A'}`);
      return true;
    } else {
    ...
    console.error('   ❌ Error consultando órdenes:', error.message);
    return false;
  }
}
