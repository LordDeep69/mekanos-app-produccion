 ...
 * Ejecutar: node scripts/diagnostico-db.js
 */
...
async function main() {
  ...
  clientes.forEach(c => {
    const gens = c.equipos.filter(e => e.tipo_equipo?.codigo_tipo === 'GENERADOR').length;
    const bombs = c.equipos.filter(e => e.tipo_equipo?.codigo_tipo === 'BOMBA').length;
    console.log(`  ID: ${c.id_cliente} | ${(c.nombre_comercial || '').substring(0, 30)} | Gens: ${gens}, Bombas: ${bombs}`);
  });
